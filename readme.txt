=== Chatme.im Status Widget ===
Contributors: camaran
Donate link: http://www.chatme.im
Tags: chat, jabber, jappix, mini, xmpp, status, widget
Requires at least: 3.3
Tested up to: 3.4
Stable tag: 0.1 beta 1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

This plugin add a widget to show ChatMe.im user status 

== Description ==

This plugin add a widget to show ChatMe.im user status

For more information, check out [chatme.im](http://chatme.im/) and [webchat](https://webchat.chatme.im/).

Special Thanks:

* Neustradamus for french translation
* To my professor for moral support
* To my sister for having tolerated
* My doctor for support
* My work for the economic support

Hand-crafted, and made with love, in [Rosolina](http://rosolina.me), Italy.

Based on [Jappix](https://project.jappix.com/) and [Jappix Mini](https://mini.jappix.com/).

You can contrib in [GitHub](https://github.com/camaran/chatme.im-mini)

== Installation ==

This section describes how to install the plugin and get it working.

1. Upload this directory to your plugins directory. It will create a 'wp-content/plugins/chatmeim-status-widget/' directory or install it via the WordPress.org plugin directory.
2. Activate the plugin through the 'Plugins' menu in WordPress

== Frequently Asked Questions ==

= A question that someone might have =

An answer to that question.

== Screenshots ==

1. Status Widget

== Changelog ==

= 1.0 beta 1 =
* First public version

== Upgrade Notice ==

= 1.0 beta 1 =
* First public version